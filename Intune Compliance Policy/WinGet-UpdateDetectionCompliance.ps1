<#	
    .NOTES
        ===========================================================================
         ...
         Filename:      WinGet-UpdateDetectionCompliance.ps1
        ===========================================================================
    .DESCRIPTION
        Intune complaince script for the detection of the installation status any deployed package within the Windows Package
        Manager (WinGet) public repository (runs as SYSTEM).
        To be used as an Intune Custom Compliance policy (Windows 10 and above) script for the detection of application upgrades
        with Windows Package Manager (WinGet). This script will return a JSON output to be processed by Intune to determine compliance.
        Please use with supplied the Custom Compliance policy JSON file.
#>
...
try {
    ...
    foreach ($update in $AvailableUpdates) {
        if ($null -ne $update.Available -and $update.Available.Trim() -ne "") {
            $AppsNeedingUpdate += [PSCustomObject]@{
                Name      = $update.Name
                Id        = $update.Id
    ...
    $HasUpdates = $AppsNeedingUpdate.Count -gt 0

    # Output JSON for Intune compliance
    $result = @{
        AppsUpToDate = -not $HasUpdates
        AppsNeedingUpdate = $AppsNeedingUpdate
    }
    

    if ($HasUpdates) {
        Write-Host "Device is not compliant, applications with available updates found."
        # AppsUpToDate is false, meaning updates are needed
        return $result | ConvertTo-Json -Compress
    } else {
        Write-Host "Device is compliant, no applications with available updates found."
        # AppsUpToDate is true, meaning no updates are needed
        return $result | ConvertTo-Json -Compress
    }
